...
namespace viator::laf
{
    ...
    public:
                              ...
                              juce::Slider &slider) override
        {
            ...
            auto b = juce::Rectangle<float>(static_cast<float>(x), static_cast<float>(y), static_cast<float>(width), static_cast<float>(height)).reduced(static_cast<float>(width) * 0.18f);
            ...
            g.setFont(font);
            g.drawText(text, 0, height - juce::roundToInt(font_size), width, juce::roundToInt(font_size), juce::Justification::centredBottom);
        }
    };
}